...
RSpec.describe Rack::ReverseProxy do
  ...
  describe "as middleware" do
    ...
    it "sets the Host header w/o default port" do
      ...
      get '/test/stuff'
      expect(a_request(:get, 'http://example.com/test/stuff').with(:headers => {"Host" => "example.com"})).to have_been_made
    end

    it "sets the X-Forwarded-Host header to the proxying host by default" do
      stub_request(:any, 'example.com/test/stuff')
      get '/test/stuff'
      expect(a_request(:get, 'http://example.com/test/stuff').with(:headers => {'X-Forwarded-Host' => 'example.org'})).to have_been_made
    end
    ...
    describe "with non-default port" do
      ...
      it "sets the Host header including non-default port" do
        ...
        get '/test/stuff'
        expect(a_request(:get, 'http://example.com:8080/test/stuff').with(:headers => {"Host" => "example.com:8080"})).to have_been_made
      end
    end
    ...
    describe "with preserve host turned off" do
      ...
      it "does not set the Host header" do
        stub_request(:any, 'example.com/test/stuff')
        get '/test/stuff'
        expect(a_request(:get, 'http://example.com/test/stuff').with(:headers => {"Host" => "example.com"})).not_to have_been_made
    ...
    describe "with x_forwarded_host turned off" do
      ...
      it "does not set the X-Forwarded-Host header to the proxying host" do
        stub_request(:any, 'example.com/test/stuff')
        get '/test/stuff'
        expect(a_request(:get, 'http://example.com/test/stuff').with(:headers => {'X-Forwarded-Host' => 'example.org'})).not_to have_been_made
        expect(a_request(:get, 'http://example.com/test/stuff')).to have_been_made
      end
    end
    ...
    describe "with a https route" do
      ...
      it "sets the Host header w/o default port" do
        ...
        get '/test/stuff'
        expect(a_request(:get, 'https://example.com/test/stuff').with(:headers => {"Host" => "example.com"})).to have_been_made
      end
    end
    ...
    describe "with a https route on non-default port" do
      ...
      it "sets the Host header including non-default port" do
        ...
        get '/test/stuff'
        expect(a_request(:get, 'https://example.com:8443/test/stuff').with(:headers => {"Host" => "example.com:8443"})).to have_been_made
      end
    end
